...
Streamlit | Professional Dark/Light Theme | SOC Analyst Grade
...
def load_css():
    ...
    <style>
    /* ── Base ── */
    .stApp { background:#0f1117; font-family:'Inter','Segoe UI',sans-serif; color:#e2e8f0; }
    .stApp h1,h2,h3 { color:#f0f4ff; }
    ...
    [data-testid="stMetric"]:hover { border-color:#4a5568; transform:translateY(-2px); }
    [data-testid="stMetric"] label { color:#718096 !important; font-size:.7rem !important;
                                      text-transform:uppercase; letter-spacing:.06em; }
    [data-testid="stMetric"] [data-testid="stMetricValue"] { color:#f0f4ff !important; font-size:1.8rem !important; font-weight:700; }
    ...
    .badge-low      { background:#14532d; color:#86efac; border:1px solid #22c55e;
                      padding:3px 10px; border-radius:20px; font-size:.72rem; font-weight:700; }
    .badge-clean    { background:#1e3a5f; color:#93c5fd; border:1px solid #3b82f6;
                      padding:3px 10px; border-radius:20px; font-size:.72rem; font-weight:700; }

    /* ── Alert cards ── */
    .alert-card {
        background:#1e2433; border-radius:10px; padding:1rem 1.25rem;
        margin:.5rem 0; border-left:4px solid;
        border-top:1px solid #2d3748; border-right:1px solid #2d3748; border-bottom:1px solid #2d3748;
    }
    ...
    /* ── Campaign cards ── */
    .campaign-card {
        background:#1a1f35; border:1px solid #3b4a6b; border-radius:10px;
        padding:1rem 1.25rem; margin:.5rem 0;
    }

    /* ── TI badge ── */
    .ti-malicious { background:#5b1a1a; color:#fca5a5; padding:2px 8px;
                    border-radius:12px; font-size:.7rem; font-weight:600; }
    .ti-suspicious{ background:#3d2608; color:#fdba74; padding:2px 8px;
                    border-radius:12px; font-size:.7rem; font-weight:600; }
    .ti-clean     { background:#0f2b0f; color:#86efac; padding:2px 8px;
                    border-radius:12px; font-size:.7rem; font-weight:600; }

    /* ── Section separator ── */
    .sep { border-top:1px solid #2d3748; margin:1.5rem 0; }

    /* ── Tabs ── */
    [data-testid="stTabs"] button { color:#718096 !important; font-size:.82rem; }
    [data-testid="stTabs"] button[aria-selected="true"] { color:#93c5fd !important;
        border-bottom:2px solid #3b82f6 !important; }

    /* ── DataFrames ── */
    [data-testid="stDataFrame"] { background:#1e2433; border-radius:10px; border:1px solid #2d3748; }

    /* ── Progress bar for FP score ── */
    .fp-bar-wrap { background:#1e2433; border-radius:4px; height:6px; width:100%; margin-top:4px; }
    .fp-bar      { height:6px; border-radius:4px; }

    /* ── Footer ── */
    .footer {
        text-align:center; color:#4a5568; font-size:.72rem;
        padding:1.5rem; margin-top:2rem; border-top:1px solid #1e2433;
    }

    /* ── Scrollable code block ── */
    .code-block {
        background:#111827; border:1px solid #1f2937; border-radius:8px;
        padding:.75rem 1rem; font-family:'JetBrains Mono','Fira Code',monospace;
        font-size:.75rem; color:#d1fae5; overflow-x:auto;
    }
    </style>
...
def layout_chart(fig, title="", height=300):
    fig.update_layout(
        ...
        font_color=FONT_CLR, margin=dict(t=30,b=20,l=10,r=10),
...
with st.sidebar:
    ...
    ti_filter= st.selectbox("TI Reputation", ti_reps)
    ...
    st.markdown('<div class="sep"></div>', unsafe_allow_html=True)
    st.markdown(
        '<div style="color:#4a5568;font-size:.7rem">SOC AI v3.0 · Rules + LLM + TI + SOAR<br>'
        'Anti-hallucination · Feedback Loop · Multi-log Correlation</div>',
        unsafe_allow_html=True
    )
...
# ─────────────────────────────────────────────
st.markdown(
    '<h1 style="margin-bottom:.25rem">🛡️ SOC AI Analyst Dashboard</h1>'
    '<p style="color:#718096;margin-bottom:1.5rem">Enterprise Threat Detection · v3.0 · '
...
k4.metric("⬆️ Escalated L2",    f"{n_escalated:,}")
k5.metric("🔗 Campaigns",       f"{n_campaigns:,}")
k6.metric("🧠 TI Malicious",    f"{n_ti_bad:,}")
...
with tab_overview:
    ...
    with c1:
        ...
        if not sev_counts.empty:
            ...
            fig = px.pie(sev_counts, values="Count", names="Severity",
                         color="Severity", color_discrete_map=color_map, hole=.45)
    ...
    with c3:
        ...
        if not outcome_counts.empty:
            ...
            fig3 = px.pie(outcome_counts, values="Count", names="Outcome",
                          color="Outcome", color_discrete_map=ocmap, hole=.45)
    ...
    with c4:
        ...
        ip_counts.columns = ["IP", "Alerts"]
        if not ip_counts.empty:
            fig4 = px.bar(ip_counts, x="Alerts", y="IP", orientation="h",
                          color="Alerts", color_continuous_scale=["#1a3a2a","#22c55e","#ef4444"])
    ...
    with c5:
        ...
        if not risk_data.empty:
            fig5 = px.histogram(risk_data, nbins=20,
                                ...
                                labels={"value":"Risk Score","count":"Count"})
    ...
    with c6:
        st.markdown("#### Log Type × Alert Status")
        ...
        fig6 = px.bar(type_alert, x="log_type", y="count", color="Status",
                      ...
                      barmode="stack",
                      labels={"log_type":"Log Type","count":"Count"})
        fig6 = layout_chart(fig6, height=280)
        st.plotly_chart(fig6, use_container_width=True)

    with c7:
        st.markdown("#### TP / FP / Uncertain")
...
# ══════════════════════════════════════════════
with tab_alerts:
    st.markdown(f"#### Alert Investigation Table — {len(df_alerts)} alerts")
    ...
    if alert_ids:
        ...
        with col_l:
            ...
            if sel_row.get("mitre_tactic"):
                st.markdown(f"**MITRE:** `{sel_row.get('mitre_tactic')}` / `{sel_row.get('mitre_t_id','N/A')}`")
            if sel_row.get("ti_reputation"):
                st.markdown(f"**Threat Intel:** {ti_badge(sel_row.get('ti_reputation',''))}", unsafe_allow_html=True)
        ...
        with col_r:
            ...
            if sel_row.get("escalate"):
                st.error(f"⬆️ **ESCALATE TO SOC LEVEL 2**\n\n{sel_row.get('esc_reason','')}")
...
with tab_campaigns:
    ...
    st.markdown("*IPs triggering correlated multi-step attacks*")
    ...
    if campaigns:
        ...
        for camp in sorted(campaigns, key=lambda x: -x.get("correlation_risk_score",0)):
            ...
            with st.expander(
                f"🔗 {camp.get('source_ip','?')} — Risk {risk}/100 — "
                f"{len(indicators)} indicator(s): {', '.join(indicators[:2])}"
            ):
                ...
                with cc1:
                    ...
                    st.markdown(f"**Has Successful Request:** `{camp.get('has_successful_request')}`")
                ...
                with cc2:
                    ...
                    for ind in indicators:
                        ...
                        st.markdown(
                            ...
                            f'border-radius:12px;font-size:.75rem;margin:2px;display:inline-block">'
                            f'{ind}</span>',
                            unsafe_allow_html=True
                        )
        ...
        st.markdown("#### Campaign Risk Overview")
...
with tab_mitre:
    ...
    if not df_alerts.empty and df_alerts["mitre_tactic"].notna().any():
        ...
        st.info("No MITRE data available in current results.")
...
with tab_ti:
    ...
    ti_m1.metric("IPs Enriched",   len(ti_all["source_ip"].dropna().unique()))
    ...
    if not ti_alerts.empty:
        ...
        for _, row in ti_display.iterrows():
            ...
            with st.expander(f"{row['source_ip']} — {row.get('attack_types','N/A')} — Score {row.get('ti_score',0)}/100"):
                ...
                tc1.markdown(f"**IP:** `{row['source_ip']}`")
                tc1.markdown(f"**Reputation:** {rep_html}", unsafe_allow_html=True)
                tc1.markdown(f"**TI Score:** `{row.get('ti_score',0)}/100`")
        ...
        if not ti_score_data.empty:
            fig_ti = px.bar(
                ...
                labels={"source_ip":"IP","ti_score":"Threat Score"},
            )
            fig_ti = layout_chart(fig_ti, "Threat Intel Score by IP", height=300)
            st.plotly_chart(fig_ti, use_container_width=True)
    else:
        st.info("No threat intel hits in current results. (Set ABUSEIPDB_API_KEY env var for remote TI.)")
...
# ══════════════════════════════════════════════
with tab_soar:
    st.markdown("#### ⚙️ SOAR Automated Response")
    st.markdown("*Security Orchestration, Automation and Response playbooks*")
    ...
    s4.metric("Avg SLA (min)",          int(soar_alerts["soar_sla"].mean()) if not soar_alerts.empty else "N/A")
    ...
    if not soar_alerts.empty:
        ...
        ticket_display = ticket_display.rename(columns={
            ...
            "source_ip":"IP","soar_playbook":"Playbook","soar_escalate":"Auto-Escalate",
            "soar_sla":"SLA (min)",
        })
        ...
        fig_pb = layout_chart(fig_pb, "Playbooks Triggered", height=max(200, len(pb_counts)*40))
        st.plotly_chart(fig_pb, use_container_width=True)
    else:
        st.info("No SOAR responses recorded. Re-run the pipeline with soar_engine enabled.")
...
with tab_eval:
        ...
        if scored:
            ...
            if not score_df.empty:
                ...
                fig_sc = px.bar(
                    ...
                    range_y=[0,10],
                    labels={"score_10":"Quality (/10)","result_id":"Alert ID"},
                )
                ...
                fig_sc2 = px.scatter(
                    ...
                    labels={"fp_score":"FP Score","score_10":"Quality (/10)"},
                    size=[8]*len(score_df),
                )
                fig_sc2 = layout_chart(fig_sc2, "Quality vs FP Score", height=300)
                st.plotly_chart(fig_sc2, use_container_width=True)

                # Breakdown table for worst alerts
                st.markdown("#### Low-Score Alert Breakdown")
                ...
                for w in worst:
                    with st.expander(f"[{w.get('score_10',0)}/10] Log#{w.get('log_index')} — {w.get('alert_type','')} — {w.get('source_ip','')}"):
                        ...
                        bd_df["Pct"] = (bd_df["Score"] / bd_df["Max"] * 100).round(0)
                        st.dataframe(bd_df, use_container_width=True, hide_index=True,
                                     column_config={
                                         "Pct": st.column_config.ProgressColumn(min_value=0, max_value=100, format="%d%%"),
                                     })
    else:
    ...
    if h_stats:
        ...
        if not h_alerts_df.empty:
            ...
            h_display = h_display.rename(columns={"h_score":"H-Score","h_flags":"Flags"})
...
with tab_feedback:
    ...
    st.markdown("*Override AI classifications. Feedback is used to calibrate FP scoring.*")
    ...
    if fb_available:
        ...
        f2.metric("Analyst TPs",      fb_stats.get("analyst_tps",0))
        f3.metric("Analyst FPs",      fb_stats.get("analyst_fps",0))
        ...
        st.warning("feedback_loop.py not found or failed to import. Ensure it's in the same directory.")
...
st.markdown(
    f"""
    <div class="footer">
        SOC AI Analyst Dashboard v3.0 — Enterprise Edition<br>
        Hybrid Rule Engine + LLM (Llama3) + Threat Intel + SOAR + Feedback Loop + Multi-log Correlation<br>
        Anti-hallucination · MITRE ATT&CK · RAG ChromaDB · Supervised Learning<br>
